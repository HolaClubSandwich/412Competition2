...
def scan_callback(msg):
    ...
    range_front = msg.ranges[340:380]  
    # range_front[5:] = msg.ranges[360:380]
    ...
    min_range,i_range = min( (ranges[i_range],i_range) for i_range in list(range(len(ranges))) )
    min_front,i_front = min( (range_front[i_front],i_front) for i_front in list(range(len(range_front))) )
    min_right,i_right = min( (range_right[i_right],i_right) for i_right in list(range(len(range_right))) )
    min_left ,i_left  = min( (range_left [i_left ],i_left ) for i_left  in list(range(len(range_left ))) )
...
scan_sub = rospy.Subscriber('/kobuki/laser/scan', LaserScan, scan_callback)   # to read the laser scanner
